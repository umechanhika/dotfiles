...
### STEP 0: 引数の解釈
- `/review-doc @path` のファイルパスを**絶対パス**に解決（`<SKILL_DIR>` はこの SKILL.md があるディレクトリ）。
- 拡張子が `md` / `markdown` / `html` / `htm` でなければ中断。
- **作業ディレクトリは指定しない**（既定で `~/.claude/review-doc/<対象パスのハッシュ>` を使う。リポジトリ内を汚さず、同じファイルを再度開くと過去スレッドを復元する）。
...
- 作業ファイル（threads.json/inbox.jsonl/server.*）は `~/.claude/review-doc/` 配下に作られ、対象リポジトリは汚さない。
