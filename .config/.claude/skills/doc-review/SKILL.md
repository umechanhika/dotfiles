...
description: >
  ...
  直前のコメント送信時点との変更点をGitHub風に赤/緑で色分け表示する
  「差分モード」も切り替えられる（HTML対象はプレビューの見た目そのものに
  色を重ねて表示）。
  「/doc-review @ファイル」で起動。
...
マークダウン/HTML ファイルをローカルのブラウザでプレビュー表示し、テキスト選択やブロック単位（表は**セル単位**、または表全体）で**Figmaのようにコメント**を付けられる。コメントは**スレッド**（コメント＋やり取り＋解決状態）として扱う:
...
- Claude の編集が反映されると自動的に**差分モード**に切り替わり、直前のコメント送信時点との変更点を GitHub 風に赤/緑で色分け表示する（HTML 対象はプレビューの見た目の上に直接色を重ねる）。上部バーのボタンでいつでも通常表示と往復できる。
...
- 作業ファイル（threads.json/inbox.jsonl/baseline.json/server.*）は `~/.claude/doc-review/` 配下に作られ、対象リポジトリは汚さない。
- 差分モード（上部バーの「変更点を表示」）の比較対象は**直前のコメント送信時点**のみで、それより前の履歴は保持しない（マークダウン・HTML 共通）。
