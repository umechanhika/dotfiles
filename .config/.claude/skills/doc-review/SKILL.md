...
description: >
  ...
  マークダウン対象では、直前のコメント送信時点との変更点をGitHub風に赤/緑で
  色分け表示する「差分モード」も切り替えられる。
  「/doc-review @ファイル」で起動。
...
- （マークダウン対象のみ）Claude の編集が反映されると自動的に**差分モード**に切り替わり、直前のコメント送信時点との変更点を GitHub 風に赤/緑で色分け表示する。上部バーのボタンでいつでも通常表示と往復できる。
...
- 差分モード（上部バーの「変更点を表示」）はマークダウン対象のみ。比較対象は**直前のコメント送信時点**のみで、それより前の履歴は保持しない。HTML 対象では切替ボタンごと出さない。
