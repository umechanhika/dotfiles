...
description: >
  ...
  デザイナーへの改善フィードバックレポート（HTML）を生成し、Notionページへ自動投稿するスキル。
...
`references/naming-patterns.md` の「NGパターン」と「状態キーワード辞書」を参照すること。
...
mkdir -p {カレントディレクトリ}/tmp/figma-mcp-readability/screenshots

### Step 2: get_screenshot を実行
...
取得した画像データを以下の命名規則でファイルに保存する:
  overview.png     → 評価対象ノード全体
  axis1_1.png      → 軸1の1件目
  axis1_2.png      → 軸1の2件目
  ...
  axis2_1.png      → 軸2の1件目
...
保存先: {カレントディレクトリ}/tmp/figma-mcp-readability/screenshots/
...
フェーズ3が完了したら、以下の順で出力する:

1. `HTMLレポートを生成しました: [ファイルパス]`

2. Notionページへの投稿を提案する:
   ```
   NotionにFBレポートを投稿しますか？
   投稿先のNotionページID（またはURL）を教えてください。
   ```
   ユーザーがNotionページID/URLを提供した場合:
   - `mcp__claude_ai_Notion__notion-create-pages` を使ってNotionページを作成する
   - 内容: タイトル「Figma MCP Readability チェック — [ページ名] ([日付])」、本文に問題カテゴリ別の指摘（Figmaノードへの直接リンク付き）
   - 作成したNotionページのURLをユーザーに返す
