...
```
⚠️ 絶対制約（このブロックを必ず最初に読むこと）:
──────────────────────────────────────────────────────
Bash コマンドおよび Python スクリプトの実行は、このフェーズでは厳禁です。
理由: 実行するとユーザーへの承認プロンプトが発生してスキルが中断します。
代わりに: XMLは get_metadata の返り値テキストをそのまま目視でスキャンしてください。
「Pythonで解析したほうが速い」と感じても、実行してはいけません。
ファイルを読む場合は Read ツールのみ使用してください。
──────────────────────────────────────────────────────
...
### XMLをBash/Pythonなしで分析する方法（重要）

get_metadata の返り値は XML テキストです。大量のノードが含まれても、以下のように目視スキャンするだけでOKです。
全件を分析する必要はありません。各軸で5件見つけたら十分です。

- **命名問題（軸1）**: テキスト内で `name="Frame ` / `name="Group ` / `name="Rectangle ` などのパターンを探す。上から順番に見て最初に見つかった5件を選ぶ。
- **コンポーネント率（軸2）**: ある `<frame` の直接子要素を確認し、`<instance` が少なく `<frame` `<group` `<vector` が多いフレームを探す。
- **状態（軸3）**: `name=` に `/` が含まれる `<frame` を探し、同じプレフィックスのグループをまとめて確認する。
- **セクション外（軸4）**: `<section` タグの外側に直接 `<frame` が存在する箇所を探す。

XMLが非常に長い場合も、上から読んでいって最初に見つかった問題ノードを5件選べば十分です。全体を走査しなくてよい。
