...
description: >-
  ...
  （hook / settings.json / skill / サブエージェント / CLAUDE.md）に変換した改善案＋実行計画ドラフトを作る。
  適用は /token-reducer-apply（sonnet）で行う。
model: opus
---
...
そのうえで16観点を一巡し、各観点を**見落とし防止のレンズ**として使う。
...
| 16 | スキルが主に名前指定で起動 → description のトリガー文要否を確認 | `signals.name_only_invocations`（名前指定が過半数 ＋ `description_chars` 大 → 自律提案が必要か確認判定） |
...
- **description トリガー文の要否確認（観点16）**：名前指定での起動が多いスキルは description のトリガー文が使われていない可能性がある。ただし**削除すると Claude の自律提案（「コミットしますね」等）も失われる**。削除が有効なのは「その自律提案が不要」と判断した場合のみ。改善案は「削除を推奨」ではなく「自律提案が必要かどうかをユーザーに確認する」止まりとし、不要なら description をスリム化する。
...
固定費（digest 読込＋16観点一巡）は1回だけ払って全案を軽量提示し、高コストな deep-read／ドラフト生成は
...
16観点を一巡し、該当なしの観点は「該当なし」と明記。価値ある提案が一つも無ければ「特になし」。
※ ここでは deep-read もドラフトコード片生成もしない。

（ユーザーがどの案を詳細化するか選択 → 段階2へ）

## 段階2: 選択された案の詳細化＋実行計画ドラフト
選ばれた finding だけ、以下を提示する:
- 原因（必要時のみ候補セッションを deep-read して確認）→ 変換先メカニズム
  （hook/settings/skill・スクリプト/サブエージェント/CLAUDE.md/LSP 設定）→ 適用ファイル/設定のドラフト（コード片）→ 静的削減見積り。
- 実行計画: 適用先（repo / dotfiles global）・適用順序・破壊的変更の有無・依存関係・トークンインパクト順。
- 検証方法: (a) 提案時の静的見積り、(b) 適用後 --split-date による縦断比較（手順は reference.md）。
※ **このターンでは実際の適用は行わない**。実行計画ドラフトの提示がこの段階の成果物。
```

## 実行計画の提示後（ハンドオフ）

- 段階1のランクリスト提示後、**どの案を詳細化するかをユーザーに確認**する（AskUserQuestion 等）。選ばれた案だけ段階2へ進める。
- 段階2で実行計画ドラフトを提示したら**このターンを終え**、以下を案内する：
  「この実行計画でよければ `/token-reducer-apply` を実行してください。適用は sonnet で行います。」
- **実際の適用・ファイル編集・apply_log 追記は `/token-reducer-apply` で行う**（このスキルでは適用しない）。
