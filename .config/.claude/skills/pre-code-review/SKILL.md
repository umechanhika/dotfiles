...
1. **md を再 Read する（必須・直前）**
   コンテキストに残っている版を信用せず、対象の md ファイル（STEP 6 で出力したもの）を **必ず Read し直す**。レビュー生成後にユーザーが文面を編集している可能性があるためで、特に system-reminder に「opened/編集中」が出ている場合は「ユーザーが編集した前提」で読み直す。以降の表・本文・API ペイロードは、この再 Read した md からのみ組み立てる。
...
7. **`AskUserQuestion` で承認を得る（必須）**
   ...
   2. **一部修正して投稿**: 修正対象の番号と修正内容（文面の修正／行の修正／指摘の削除）を指定してもらう。適用後、**md ファイルも同じ内容に更新**し、手順1（md 再 Read）からやり直して表・全文プレビューを再提示し、再度この承認を取る（承認が出るまでループ）。行を直した場合は投稿方式を再判定する。
