...
`{title}` は STEP 1〜4 のスクリプト出力マニフェスト `=== PR ===` の `title=` 値をそのまま使う（追加の `gh` 呼び出し不要）。
...
- 「コメントを投稿する場合は指示してください」と添える（投稿時は後述の必須ゲートで、最新 md を再読込し全文を表でプレビューしてから承認を得て `post-review.sh` 経由で投稿する。詳細は `references/review-pr-comment.md` 参照）
